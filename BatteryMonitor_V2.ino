...
// Software Version: 2.00, 03.05.2021
...
// Power variables
int cycles=582;                      //enter numbers of cycles, you want to flash on EEPROM first time
byte flashcyclesLobyte;
byte flashcyclesHibyte;
unsigned int battcapacity=8426;      //enter your battery capacity in Wh (max. 65300), you want to flash on EEPROM first time 
...
unsigned long ExtWDTinterval=2500;      // interval of WDT reset within loop in mSec
...
#define Testswitch 23                   // enter your Switch Digital Pin
byte Mode;
...
byte EEPROMvalue[5];            // Array accomodating 5 stored values, increase if needed
...
//  Serial.begin(9600);
  ...
  for (EEPROMaddress=0;EEPROMaddress<5;EEPROMaddress++) {
  ...
  if (EEPROMvalue[0]==255 or EEPROMvalue[0]==0) {
    ...
    flashbattcapacityLobyte=extractor;
     ...
     EEPROM.write(1,flashcyclesLobyte);   
     EEPROM.write(2,flashcyclesHibyte); 
     EEPROM.write(3,flashbattcapacityLobyte);   
     EEPROM.write(4,flashbattcapacityHibyte); 

  }
  else {
     cycleslobyte= EEPROMvalue[1];
     cycleshibyte= EEPROMvalue[2];
     battcapacityLobyte= EEPROMvalue[3];
     battcapacityHibyte= EEPROMvalue[4];
...
// Blynk
 ...
 if (Mode==1){
  ...
  if (subpage==1){                        // page 2
    if (capsave==0 and watthoursTot>0){
    ...
    EEPROM.write(3,battcapacityLobyte);
    EEPROM.write(4,battcapacityHibyte);
    capsave=1;
    }
    
  lcd.setCursor(0,0);
  lcd.print("new Battery Capacity");  
  lcd.setCursor(0,1);
  lcd.print("       SAVED!       "); 
  lcd.setCursor(0,2);
  lcd.print("                    "); 
...
void BlynkDataTransmit(){
 ...
 if (Mode==1){
  Blynk.virtualWrite(V6, round(ActWh)); 
 }
 else{
...
void getVoltage(){
 ...
 voltcounter++;
  ...
  if (diffbattvoltage>0.15){
  SOCfreeze=SOCfreeze+20;  
  }
...
void getDischargeData(){
 ...
 if (millis()>=(dwattsstartmillis+180000)){    // Ah and Wh calculated every 3min
  dwattsstartmillis=millis();
  oldActWh=ActWh;
  daveragewatts=(daveragewatts/dwattcounter)/(3600000/180000); 
  ActWh=ActWh+daveragewatts;
  daverageAmps=(daverageAmps/dwattcounter)/(3600000/180000); 
